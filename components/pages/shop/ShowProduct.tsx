  ...
  return (
    <div className="relative">
      <nav className="flex justify-between items-center">
        <h4 className="hidden lg:block text-2xl font-bold">{category}</h4>
        <div className="flex gap-3 text-gray-400">
          <span>
            Showing {(currentPage - 1) * limit + 1} -{" "}
            {data?.total && Math.min(currentPage * limit, total)} of {total}
          </span>
          <div className="hidden md:flex">
            <span className="mr-1">Sort by:</span>
            <SelectDemo
              options={Object.keys(selectOptionsMap)}
              onChange={(value) => setSortBy(selectOptionsMap[value])}
            />
          </div>
        </div>
        <AnimatePresence>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-5 mt-4 border-b w-full justify-center">
        {isLoading ? (
          <div className="col-span-full flex justify-center items-center h-96">
            <ShowLoader />
          </div>
        ) : isError ? (
          <div className="col-span-full flex justify-center items-center h-96"></div>
        ) : products.length === 0 ? (
          <p className="col-span-full flex justify-center items-center h-96 text-gray-500">
            No products found.
          </p>
        ) : (
          products.map((p: Product) => <Card key={p.id} {...p} />)
        )}
      </div>
